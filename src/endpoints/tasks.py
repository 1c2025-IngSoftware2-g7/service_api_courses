...
def create_task():
    ...
    if not data:
        error = error_generator(
            f"[TASKS][CONTROLLER] {MISSING_FIELDS}", "User UUID is required", 400, "get_modules_from_course"
        )
        return error["response"], error["code_status"]
...
def get_tasks_by_course(course_id=None):
    ...
    logger.debug(f"[TASKS][CONTROLLER] Getting tasks for course {course_id} with status filter: {status}. And module_id filter: {module_id}")
...
def submit_task(uuid_task):
    ...
    try:
        ...
        error = error_generator(
            "[TASKS][CONTROLLER] Bad Request", str(e), 400, f"tasks/submission/{uuid_task}"
        )
        return error["response"], error["code_status"]
        ...
        error = error_generator(
            "[TASKS][CONTROLLER] Internal Server Error", str(e), 500, f"tasks/submission/{uuid_task}"
        )
        return error["response"], error["code_status"]
