...
def remove_assistant_from_course(course_id=None):
    ...
    if not data:
        error = error_generator(
            "Missing data", MISSING_FIELDS, 400, "remove_assistant_from_course"
        )
        return error["response"], error["code_status"]

    if "assistant_id" not in data:
        error = error_generator(
            "Missing assistand id", MISSING_FIELDS, 400, "remove_assistant_from_course"
        )
        return error["response"], error["code_status"]

    if "owner_id" not in data:
        error = error_generator(
            "Missing owner id", MISSING_FIELDS, 400, "remove_assistant_from_course"
        )
        return error["response"], error["code_status"]
